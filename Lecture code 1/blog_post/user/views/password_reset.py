...
class PasswordResetRequestView(View):
    def get(self, request):
        ...
        return render(request,
                      template_name="password_reset/password_reset_request.html",
                      context={"form": form})
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            reset_link = f"http://127.0.0.1:8000/user/reset/{uid}/{token}/"

            # Send email
            subject = "Password Reset Request"
            message = render_to_string(template_name="password_reset/password_reset_email.txt",
                                       context={"user": user,
...
class PasswordResetConfirmView(View):
    def get(self, request, uidb64, token):
        ...
        if user and custom_password_reset_token.check_token(user, token):
            ...
            return render(request, template_name="password_reset/password_reset_confirm.html",
                          context={"form": form,
                                   "uidb64": uidb64,
                                   "token": token
                                })
